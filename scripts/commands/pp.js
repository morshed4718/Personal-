...
const fs = global.nodemodule["fs-extra"];
  ...
  var callback = (✅) =>   api.sendMessage({body:`==profile==â”`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + /cache/1png),event.messageID); 
    return request(encodeURI(`https://graph.facebook.com/${id}/height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',
        () => callback()); 
    }
    if (!args[0]) {
    ...
    return request(encodeURI(`https://graph.facebook.com/$event.senderID/picture?height=1500&width=1500&access_token=66268379%7Cc1e620fa708a1d5696fb991c1bde662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',
        () => callback()); 
    }
    else {
...
var data = await api.getUserInfoV2(res_ID);
    ...
    var callback = () => api.sendMessage({body:`=== [ profile==]`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
    return request(encodeURI(`https://graph.facebook.com/${res_ID}/picture?height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde662`)).pipe(fs.createWriteStream(__dirname+'/cache/1png')).on('close',
        () => callback()); }
  else {
